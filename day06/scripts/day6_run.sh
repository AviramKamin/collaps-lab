...
# ---------------------------
# Day 6 Runner (Recovery Fingerprint)
# Cycles: Baseline -> Intervention -> ExtendedRecovery -> PostBaseline
# Adds meminfo sampling as hidden-state probe
# Writes logs to ../runs/<timestamp>_day6_offX_nY/
# ---------------------------
...
RECOVERY_EXT_SEC="${RECOVERY_EXT_SEC:-600}"    # longer than Day 5
...
# Meminfo fields (space-separated). Keep minimal and stable.
MEMINFO_FIELDS="${MEMINFO_FIELDS:-MemAvailable Dirty Writeback Slab}"

# ---- Run directory ----
TS="$(date +%Y-%m-%d_%H%M%S)"
RUN_NAME="${TS}_day6_off${OFF_SEC}_n${N_CYCLES}"
RUN_DIR="${RUNS_DIR}/${RUN_NAME}"
mkdir -p "$RUN_DIR"

echo "[INFO] Run dir: $RUN_DIR"
echo "[INFO] Cycles: $N_CYCLES | OFF=${OFF_SEC}s | HB=${HB_INTERVAL}s | MEMINFO=${MEMINFO_INTERVAL}s"
echo "[INFO] Baseline=${BASELINE_SEC}s Intervention=${INTERVENTION_SEC}s RecoveryExt=${RECOVERY_EXT_SEC}s PostBaseline=${POSTBASELINE_SEC}s"
...
RECOVERY_EXT_SEC=$RECOVERY_EXT_SEC
...
# <ts_ns> MemAvailable_kB=... Dirty_kB=... Writeback_kB=... Slab_kB=...
meminfo_logger() {
  local interval="$1"
  local fields="$2"
  while true; do
    ...
    # Parse only requested fields from /proc/meminfo.
    # Values are in kB in meminfo; we keep them as kB for stability.
    local line
    line="$(awk -v want="$fields" '
      ...
      }
      {
        key=$1; sub(/:$/,"",key);
        if (wanted[key]) {
          # $2 is numeric value, $3 is unit (usually kB)
          printf "%s_kB=%s ", key, $2;
        }
      }
      END{ print "" }
...
cleanup() {
  ...
  if [[ -f "$MI_PIDFILE" ]]; then
    kill "$(cat "$MI_PIDFILE")" 2>/dev/null || true
  fi
}
trap cleanup EXIT
...
for c in $(seq 1 "$N_CYCLES"); do
  ...
  # Extended recovery
  mark "C${c}_RECOVERY_EXT_START"
  sleep "$RECOVERY_EXT_SEC"
  mark "C${c}_RECOVERY_EXT_END"
