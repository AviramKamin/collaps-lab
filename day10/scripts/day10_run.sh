...
# ---------------------------
# Day 10 Runner (Probe Stall Attribution: Decomposition + R2 Telemetry)
# Cycle: B -> I -> R1 -> R2(probes by MODE + PROBE_ACTIONS, telemetry) -> B2
# Writes logs to ../runs/<timestamp>_day10_offX_<MODE>_actions<PROBE_ACTIONS>_nY/
# ---------------------------
...
PROBE_PROGRAM="${PROBE_PROGRAM:-low}"   # control|low|high|late (Day10 expected: low)
PROBE_ACTIONS="${PROBE_ACTIONS:-ABC}"   # A|B|C|ABC (Day10 main axis)
...
RUN_NAME="${TS}_day10_off${OFF_SEC}_${MODE}_actions${PROBE_ACTIONS}_n${N_CYCLES}"
...
echo "[INFO] Probes: program=$PROBE_PROGRAM | ENABLE_PROBES=$ENABLE_PROBES | actions=$PROBE_ACTIONS"
...
probe_action_B() {
  ...
  # NOTE: keep sync behavior identical to Day9 so comparisons remain valid
  sync
}
...
run_r2_probes() {
  ...
  # late program preserved (Day10 expected to use low, but keep compatibility)
