...
BASE_DIR="${BASE_DIR:-$HOME/project/day20}"
CONDITION="${CONDITION:-A}"
SCRIPTS_DIR="${SCRIPTS_DIR:-$(pwd)}"
RUN_TAG="${RUN_TAG:-storage_vs_retry}"
...
RETRIES="${RETRIES:-5}"
ENABLE_INTERVENTION_IO="${ENABLE_INTERVENTION_IO:-1}"

# Paths
BGIO_FILE="${BGIO_FILE:-$SCRIPTS_DIR/day20_bgio.fio}"
DISK_RETRY_TARGET_DEFAULT="${DISK_RETRY_TARGET_DEFAULT:-$BASE_DIR/workdir/retry_test}"
TMPFS_RETRY_TARGET_DEFAULT="${TMPFS_RETRY_TARGET_DEFAULT:-/dev/shm/day20_retry_test}"
...
RUN_ID="$(date +%F_%H%M%S)_day20_${CONDITION}_${RUN_TAG}"
...
{
    echo "[INFO] Starting Day20 run: $RUN_ID"
...
for (( CYCLE=1; CYCLE<=N_CYCLES; CYCLE++ )); do
    ...
    if [[ "$ENABLE_INTERVENTION_IO" -eq 1 ]]; then
        ...
        echo "[INFO] Intervention IO disabled for CONDITION=$CONDITION" | tee -a "$LOG"
...
echo "[INFO] Day20 run complete: $RUN_DIR" | tee -a "$LOG"