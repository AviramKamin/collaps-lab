...
# ---------------------------
# Day15 Runner (Kernel Execution Attribution During Latency Spikes)
# Cycle: B -> I -> R1 -> R2(probe B fsync only + fine telemetry + disk watch + kernel trace) -> B2
# Focus: kernel-path attribution during persistent latency spikes
# ---------------------------
...
# ---- Day15 Locked Probe Settings ----
# Keep the known reproducer stable: probe B, low program, fsync; default to tmpfs probe root
PROBE_PROGRAM="${PROBE_PROGRAM:-low}"          # control|low
PROBE_ACTIONS="B"                              # locked
PROBE_B_VARIANT="${PROBE_B_VARIANT:-fsync}"
PROBE_RT="${PROBE_RT:-0}"
PROBE_RT_PRIO="${PROBE_RT_PRIO:-90}"

PROBE_TMPFS_DIR="${PROBE_TMPFS_DIR:-/dev/shm/probes}"
PROBE_FILE_MB="${PROBE_FILE_MB:-64}"
PROBE_ROOT="${PROBE_ROOT:-/dev/shm/day15_probes}"
mkdir -p "$PROBE_ROOT"
...
# ---- Kernel tracing (Day15) ----
ENABLE_TRACE="${ENABLE_TRACE:-1}"
TRACE_SCRIPT="${TRACE_SCRIPT:-$SCRIPTS_DIR/kernel_trace_control.sh}"
TRACE_EVENTS="${TRACE_EVENTS:-sched:sched_switch,sched:sched_wakeup,sched:sched_wakeup_new}"
TRACE_OUT_NAME="${TRACE_OUT_NAME:-trace.log}"
...
RUN_NAME="${TS}_day15_${MOUNT_LABEL}_off${OFF_SEC}_bvar${PROBE_B_VARIANT}_n${N_CYCLES}"
...
echo "[INFO] Trace: ENABLE_TRACE=$ENABLE_TRACE script=$TRACE_SCRIPT events=$TRACE_EVENTS"
...
start_trace() {
  ...
  if [[ "$ENABLE_TRACE" != "1" ]]; then
    echo "$(date +%s%N) TRACE_DISABLED" >> "$cycle_dir/$TRACE_OUT_NAME"
    return
  fi
  if [[ ! -x "$TRACE_SCRIPT" ]]; then
    echo "[ERROR] Trace script not executable: $TRACE_SCRIPT"
    exit 1
  fi
  local out="$cycle_dir/$TRACE_OUT_NAME"
  ( cd "$SCRIPTS_DIR" && env TRACE_EVENTS="$TRACE_EVENTS" bash "$TRACE_SCRIPT" start "$out" )
}
...
stop_trace() {
  ...
  local out="$cycle_dir/$TRACE_OUT_NAME"
  ( cd "$SCRIPTS_DIR" && env TRACE_EVENTS="$TRACE_EVENTS" bash "$TRACE_SCRIPT" stop "$out" ) || true
}
...
for c in $(seq 1 "$N_CYCLES"); do
  ...
  start_trace "$CYCLE_DIR"
  start_interference "$CYCLE_DIR"

  local_half=$(( INTERVENTION_SEC / 2 ))
  local_rest=$(( INTERVENTION_SEC - local_half ))

  sleep "$local_half"

  # Split the intervention trace into two back-to-back segments so the trace
  # buffer does not overwrite the early intervention window before stop.
  stop_trace "$CYCLE_DIR"
  start_trace "$CYCLE_DIR"
