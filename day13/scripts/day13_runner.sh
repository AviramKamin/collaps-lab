...
# ---------------------------
# Day13 Runner (Journal vs Flush Attribution)
# Cycle: B -> I -> R1 -> R2(probe B fsync only + telemetry) -> B2
# Focus: ext4 mount semantics sensitivity
# ---------------------------
...
# ---- Day13 Locked Probe Settings ----
# Only probe B, low program, fsync on ext4 path
PROBE_PROGRAM="${PROBE_PROGRAM:-low}"          # control|low (Day13 should be low)
PROBE_ACTIONS="B"                              # locked
PROBE_B_VARIANT="${PROBE_B_VARIANT:-fsync}"    # locked to fsync for Day13
...
RUN_NAME="${TS}_day13_${MOUNT_LABEL}_off${OFF_SEC}_bvar${PROBE_B_VARIANT}_n${N_CYCLES}"
