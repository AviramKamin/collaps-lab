...
if [[ ! -x "$SCRIPTS_DIR/retry_storm.sh" ]]; then
  echo "[ERROR] Missing or non-executable retry_storm.sh at $SCRIPTS_DIR/retry_storm.sh" >&2
  exit 1
fi
...
case "$CONDITION" in
  A)
    ...
    ;;
  B)
    COMPLETION_MODE="fdatasync"
    ARTIFICIAL_DELAY_MS="0"
    ;;
  C)
    COMPLETION_MODE="delay"
    ARTIFICIAL_DELAY_MS="${ARTIFICIAL_DELAY_MS:-10}"
    ;;
  *)
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_day26_${CONDITION}"
...
write_meta() {
  {
    ...
    echo "ARTIFICIAL_DELAY_MS=$ARTIFICIAL_DELAY_MS"
...
write_meta
echo "[INFO] Starting run $RUN_ID CONDITION=$CONDITION COMPLETION_MODE=$COMPLETION_MODE ARTIFICIAL_DELAY_MS=$ARTIFICIAL_DELAY_MS" | tee -a "$LOG"
