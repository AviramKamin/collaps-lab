...
# Observer outputs are cycle-specific to preserve all cycles
VMSTAT_LOG=""
IOSTAT_LOG=""
TRACE_DAT=""
...
start_sampling() {
    : > "$VMSTAT_LOG"
    : > "$IOSTAT_LOG"
    echo "$(date +%s%N) sampling_start interval=${OBS_SAMPLE_INTERVAL_SEC}s vmstat_log=$VMSTAT_LOG iostat_log=$IOSTAT_LOG" >> "$PROBES_LOG"
    ...
    echo "[INFO] Started bounded sampling: vmstat_pid=$VMSTAT_PID iostat_pid=$IOSTAT_PID vmstat_log=$VMSTAT_LOG iostat_log=$IOSTAT_LOG" | tee -a "$LOG"
}
...
for (( CYCLE=1; CYCLE<=N_CYCLES; CYCLE++ )); do
    # Cycle-specific observer artifacts
